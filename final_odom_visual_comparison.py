...
first_matrix_orig = np.identity(4)
first_point_orig = np.array([[0, 0, 0, 1]]).T 
...
for index in range(7, 3872): 
    ...
    fin_fund_matrix, inlier1, inlier2 = outlier_rejection(features1, features2, 0.15, 40)
    ...
    # # CV function
    esst_matrix, _ = cv.findEssentialMat(np.array(features1), np.array(features2), K, cv.RANSAC, threshold=1.0)
    _, R, C, _ = cv.recoverPose(esst_matrix, np.array(features1), np.array(features2), K)
    first_matrix_orig = first_matrix_orig @ final_matrix(R, C) 
    p_orig = first_matrix_orig @ first_point_orig 
    all_pts_orig.append(p_orig)
    
    plt.scatter(p[0][0], -p[2][0], color='b')
    plt.scatter(p_orig[0][0], -p_orig[2][0], color='r')

    err += np.sqrt((p_orig[0][0] - p[0][0])**2 + (p_orig[2][0] - p[2][0])**2)
